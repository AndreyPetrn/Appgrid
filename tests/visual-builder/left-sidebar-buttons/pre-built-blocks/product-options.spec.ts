...
test("Product options from pre-built block must be added to body", async ({page}) => {
    ...
    await test.step("Add 'product options 1' to body", async() => {
        await leftSidebarElements.openSidebar('pre_built');
        await leftSidebarElements.selectPreBuildBlock('Product options');
        await leftSidebarElements.addPreBuildBlockInContainer('Product option 1', selectors.body);
    ...
    await test.step("Add 'product options 2' to body", async() => {
        await leftSidebarElements.openSidebar('pre_built');
        await leftSidebarElements.selectPreBuildBlock('Product options');
        await leftSidebarElements.addPreBuildBlockInContainer('Product option 2', selectors.body);
        await builderPage.waitElementInSection(selectors.body, 'quantity-selector');
        await builderPage.waitElementInSection(selectors.body, 'variant-selector');
        await builderPage.waitElementInSection(selectors.body, 'buy-button');
        await leftSidebarElements.hotkey(data.KeyboardShortcuts.Delete);
    ...
    await test.step("Add 'product options 3' to body", async() => {
        await leftSidebarElements.openSidebar('pre_built');
        await leftSidebarElements.selectPreBuildBlock('Product options');
        await leftSidebarElements.addPreBuildBlockInContainer('Product option 3', selectors.body);
        await builderPage.waitElementInSection(selectors.body, 'variant-selector');
        await builderPage.waitElementInSection(selectors.body, 'quantity-selector');
    });
});