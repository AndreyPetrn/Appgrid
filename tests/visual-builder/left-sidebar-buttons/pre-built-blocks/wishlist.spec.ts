...
test("Wishlist from pre-built block must be added to body", async ({page}) => {
    ...
    await test.step("Add 'wishlist 1' to body", async() => {
        await leftSidebarElements.openSidebar('pre_built');
        await leftSidebarElements.selectPreBuildBlock('Wishlist');
        await leftSidebarElements.addPreBuildBlockInContainer('Wishlist 1', selectors.body);
    ...
    await test.step("Add 'wishlist 2' to body", async() => {
        await leftSidebarElements.openSidebar('pre_built');
        await leftSidebarElements.selectPreBuildBlock('Wishlist');
        await leftSidebarElements.addPreBuildBlockInContainer('Wishlist 9', selectors.body);
        await builderPage.waitElementInSection(selectors.body, 'wish-list');
    });
});