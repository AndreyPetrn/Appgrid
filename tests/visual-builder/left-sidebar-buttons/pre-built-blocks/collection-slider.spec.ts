...
test("Collection slider from pre-built block must be added to body", async ({page}) => {
    ...
    await test.step("Add 'collection slider 1' to body", async() => {
        await leftSidebarElements.openSidebar('pre_built');
        await leftSidebarElements.selectPreBuildBlock('Collection slider');
        await leftSidebarElements.addPreBuildBlockInContainer('Collection slider 1', selectors.body);
        await builderPage.waitElementInSection(selectors.body, 'slider-base');
    });
});