...
test("Product description from pre-built block must be added to body", async ({page}) => {
    ...
    await test.step("Add 'product description 1' to body", async() => {
        await leftSidebarElements.openSidebar('pre_built');
        await leftSidebarElements.selectPreBuildBlock('Product description');
        await leftSidebarElements.addPreBuildBlockInContainer('Product description 1', selectors.body);
        await builderPage.waitElementInSection(selectors.body, 'content-menu-dropdown');
    });
});